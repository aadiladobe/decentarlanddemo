...
const sphTransform = new Transform({ 
  scale: new Vector3(3, 1, 1),
 });
 sphereEntity.addComponent(sphTransform);
...
sphereEntity.addComponent(
  new OnPointerDown((e) => {
    ...
    const couponCode = commerce.getCoupon();
    ...
    const textTransform = new Transform({ 
      ...
      scale: new Vector3(0.2, 0.5, 0.5),
     });
     textEntity.addComponent(textTransform);

    if (textEntity.alive) {
      log("text entity is added to the engine")
    }
